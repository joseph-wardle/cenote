...
Parent scope is charter §4 M2: production I/O + full look. Decisions D-052…D-067 in
[decisions.md](decisions.md) carry the full rationale; this file is the working plan.
Everything consciously *not* built — the "production solution, but not yet" options —
lives in [deferrals.md](deferrals.md) with its revival trigger.*
...
| 3 | Edit model (D-054) | **Change-sets as first-class values**: ordered typed patches (`Option` per attribute), get-or-create by name on apply; `apply()` is the *only* mutation path and accumulates the dirty state that drives re-prep | A file load IS a change-set (RDL2's insight); file = wire = undo = the same value; one mutation path means one dirty-tracking story |
...
| 6 | Importer subset (D-057) | trianglemesh/plymesh/sphere/ObjectInstance; diffuse/coateddiffuse/conductor/dielectric/thindielectric; area/infinite/distant lights; imagemap/constant/scale textures; perspective camera — **all five fidelity traps handled** (see §6) | Covers the corpus that actually exists; every skipped feature warns with the token name so silence never means "handled" |
...
| 14 | Corpus (D-065) | **Tiered**: 3–4 small CC0 pbrt scenes vendored with goldens (import → render → FLIP in CI); checksummed fetch script for showcase scenes; README pins the reference pbrt-v4 commit and states the spectral-vs-RGB caveat | CI stays fast and hermetic; the showcase tier stays out of the repo |
...
- **Dependencies** (per D-011): core gains `serde`, `ron`, `image` (PNG/JPEG decode),
  ...
  pbrt tokenizer/parser (cenote-pbrt).
...
- **Color pipeline**: color textures stored BC7-sRGB, hardware sRGB decode, then the
  ...
  in-kernel pattern). Data maps stay linear.
- **Prep pipeline**: decode → mip-cap downscale → BC encode → DDS cache written next
  to the source (Cycles `blender_tx` pattern); cache hit skips everything.
...
- **Denoise cadence**: viewer toggle at ~1 Hz, BALANCED quality; CLI `--denoise`
  final frame at HIGH with prefiltered guides (`cleanAux`) — Cycles' split.
...
2. **Change-set schema + RON round-trip** — pure CPU: the value types, apply
   semantics (get-or-create, dirty accumulation), serde derives, round-trip and
   apply-order tests; `Scene::demo` re-expressed as a change-set. *Checkpoint: the
   demo scene exists as data on day one.*
...
5. **Closure**: variable IOR through the energy-compensation fits, transmission +
   thin-walled + opacity, coat with darkening, fuzz LTC, emission in its slot.
   *Checkpoint: furnace matrix extends to the new lobes' white configurations.*
...
is the closure watch item. Build-side: `intel_tex_2`'s ISPC binaries and the `oidn`
crate's runtime library discovery are checked in a spike before their steps commit
to them (their fallbacks: `ddsfile`-only uncompressed upload; CLI-only denoise).
