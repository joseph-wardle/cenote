...
/// Build the combined GPU light list: triangle records first (grouped by
/// instance, in primitive order — `GeometryRecord.light + primitive`
...
fn finish_alias(mut records: Vec<LightRecord>) -> Vec<LightRecord> {
    ...
    if total <= 0.0 {
        // Nothing here can be sampled (the all-dark case above); the
        // defaults — pdf 0, threshold 1 — already say so.
        return records;
    }
